...
export const executeCpp = async (code, input) => {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const compileCommand = `docker run --rm -v "${absoluteJobDir}:/app" cpp-runner bash -c "g++ /app/code.cpp -o /app/code.out"`;
      const runCommand = `docker run --rm -v "${absoluteJobDir}:/app" cpp-runner bash -c "/app/code.out < /app/input.txt > /app/output.txt && cat /app/output.txt"`;
